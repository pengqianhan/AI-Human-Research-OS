...
| `in-progress` | An edge into it is running |
...
An edge becomes `ready` when its source waypoint is `verified` (or is the
start). An edge is sized to one clean agent session; if its prompt cannot fit
one session, split the target waypoint.
