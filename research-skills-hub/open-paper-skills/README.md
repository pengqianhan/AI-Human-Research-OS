...
Use [research-skill-installer](research-skill-installer/SKILL.md) for commands,
target selection, and verification. Install destinations come from its target
table; symlink or copy form comes from the collection's `SOURCE.md`. See
[Managing skills](../MANAGING-SKILLS.md) for the lifecycle and source policy.
...
Maintains and uses a project-local, git-trackable map of the human's cognition.
The map scaffolds work beyond the human's current understanding: the agent owns
technical investigation and reversible implementation, the human retains
consequential decisions, and verified artifacts become the basis for targeted
teaching and evidence-backed cognitive growth. See the skill's
[README](human-cognition-cache/README.md) for the design and GPT-2 example.

Example requests:

```text
/human-cognition-cache initialize the cache in this repo
/human-cognition-cache help me complete this task beyond my current knowledge
