...
| M4 opened one narrow slice only | `os-ui` may rename `SKILL.md` ↔ `SKILL.md.disabled` to disable/enable a skill **per install location**. Chosen because the rename is non-destructive, reversible, and Git-visible. The write channel is a **Vite dev-server middleware**, alive only while `start.sh` runs — no resident process, so DESIGN.md §2's "no resident daemon" invariant is untouched. Install and delete stay copy-only. Borrowed from asm's `skill-state.json` disable mechanism | Remove the plugin from `os-ui/frontend/vite.config.ts` and the toggle from the store page; revert the DESIGN.md §1/§8 and GOAL.md M4 edits |
