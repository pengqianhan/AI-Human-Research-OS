# Task: 规范化 AI-Human Research OS 模板
...
请分析并规范化本仓库的 Research OS 设计，使其成为一个简单、高效、可演化、便于 code agent 驱动的研究工作模板。
...
请以仓库中实际存在的路径为准，并在必要时修正文档中的命名不一致问题。
...
1. 一个规范化后的 Research OS 仓库，包含上述目录结构、文件和更新后的文档，codex 和 claude code 应当能在这个OS里尽可能长时间的运行，可以用codex cli 来做smoke test。smoke test 中codex 在根目录开始，最终可以完成一个项目包括code 和 paper(main.tex，并编译出pdf)
2. 整个OS的介绍文档，html格式。
2. 一份简短的变更总结，说明改了什么、为什么改，html格式。
