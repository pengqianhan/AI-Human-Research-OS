...
1. 已更新的文档和目录说明。
2. 一份简短的变更总结，说明改了什么、为什么改。
3. 一份仍需用户决策的问题列表，例如是否需要重命名目录、是否需要 CLI、是否要引入更结构化的索引格式。
4. 如运行了检查、脚本或 lint，请列出命令和结果。
...
- 记忆机制和 skill 演化机制有明确边界，能区分项目内经验和全局通用经验。
...
1. 读取 `INSTRUCTION.md`，确认项目规则。
2. 快速审阅 `README.md`、`FILETREE.md`、`AGENTS.md`、`CLAUDE.md` 和目录结构。
3. 列出现有不一致和最小修正方案。
4. 更新文档，优先修正文档而不是重排目录。
5. 如有必要，再提出需要用户确认的目录重命名或结构调整。
6. 刷新 `FILETREE.md` 并运行对应检查。
7. 汇总变更和待决策事项。
