...
* [OpenRath](../papers/2606.19409.md) - makes memory recall/commit, tool evidence, and branch lineage explicit runtime events attached to a first-class Session value rather than hidden prompt text or side-channel bookkeeping, so what entered an agent's working context (and why) stays inspectable after the fact.
* [ContextPilot](../papers/2608.28476.md) - extends a context-editing toolset with planning, structured long-term memory, and soft (recoverable) offloading, then trains the tool-use policy itself with a partial-rollout scheme that branches extra exploration at high-sensitivity context-editing actions and a snapshot-level credit assignment that averages reward over every branched continuation.

# Synthesis

AI Agents Do Not Fail Alone treats the assembled context as a diagnostic object whose clarity, grounding, consistency, tool descriptions, safety boundaries, trust separation, and efficiency can be scored before behavioral evaluation. MemoHarness treats context construction as one controllable stage in an adaptive execution policy and uses retrieved experience to decide when a case needs richer instructions, evidence, tools, memory, or workflow. Recursive Language Models takes the opposite move from compaction: rather than deciding what to keep in context, it keeps the prompt out of context altogether as a REPL variable and lets the model query it through code. SearchOS adds a state-centric route: task progress, evidence, coverage, and failures live outside conversation, while each role receives a fresh projection of only the state it needs. LongHorizon-Harness pushes this state-centric route further by making the auditor, not the executor, the only source that can write to persistent state — discarding each round's raw trajectory outright rather than compacting or filtering it, so context rot and self-assessment propagation are avoided by construction rather than by scoring or retrieval. GenericAgent takes yet another route: rather than deciding what external state to keep or discard, it treats the *density* of the agent's own default working context as the design target, combining a deliberately small tool surface (fewer tools to describe in-context in the first place) with staged, lossy compression (truncation, tag-level summarization, FIFO eviction) bounded by continuously-reinjected anchor prompts — a within-context compaction strategy that sits at the opposite end of this topic's spectrum from Recursive Language Models' out-of-context REPL-variable approach. OpenRath approaches the topic from underneath rather than alongside these compression strategies: instead of deciding what to keep, discard, or query out-of-band, it asks what runtime object should carry context-relevant events (memory recall/commit, tool results, branch lineage) in the first place, so that whichever compression or retrieval strategy a system uses, the record of what entered context and why remains attached to an inspectable value rather than reconstructed after the fact from logs. Together they motivate measuring not only whether context is well formed, but whether the material selected for a particular case is current, relevant, trustworthy, and worth its token cost. ContextPilot approaches the topic from a different angle than any of the above: rather than proposing a new assembly, compression, or state-tracking architecture, it asks how to *train* the policy that performs context editing in the first place. Its pilot study offers direct evidence for a premise several of the above papers assume rather than measure — that context-management actions have heterogeneous downstream impact — and its fine-grained credit assignment (averaging reward over every branched continuation of a context-editing decision, proven lower-variance than trajectory-level assignment) is a training-side answer to this topic's question of what to keep, discard, or compress: sample and check what happens after each candidate action, rather than deciding by a fixed compression rule or density heuristic.
...
* ContextPilot's branch-and-average credit assignment requires sampling multiple continuations from a candidate context-editing decision during training — does this training-time signal generalize to inference-time decisions (e.g. MemoHarness's per-case context assembly, or GenericAgent's compression triggers) that must choose without the budget to sample ahead?
