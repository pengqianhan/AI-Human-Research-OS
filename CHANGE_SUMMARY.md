...
that git history alone can't explain. Forward-looking resume point:
[task_plan.md](task_plan.md). Maintained by the `session-handoff` skill.
...
| D4 | HTML as the format for operating docs | **— superseded 2026-06-17:** the living hand-off files (`task_plan`, `CHANGE_SUMMARY`) are now **Markdown**; `OS_INTRO.html` stays HTML as a static delivered intro | Convert the hand-off files back to HTML (not recommended — Markdown is cheaper to read/edit/grep/diff) |
...
| Where decisions are recorded | **Deduped to this file** (durable cross-project subset mirrored in `Memory/MEMORY.md`); `task_plan.md` only links here | Re-add a decisions table elsewhere (not recommended — invites drift) |
...
- **Root `Templates/` container** — deferred until a second template exists (see [task_plan.md](task_plan.md)).
