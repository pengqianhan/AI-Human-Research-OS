# Task: Explore and Normalize the AI-Human Research OS Template

## Background

This repository is building a lightweight Research OS for human researchers and code agents. At the moment, it is mainly a folder structure and a set of operating conventions for organizing ideas, references, experiments, figures, writing materials, memory, and reusable skills.

The goal is not to build a complex platform. The goal is to help Codex, Claude Code, and similar code agents support long-term, iterative research with lower token cost, more stable path conventions, and clearer context-loading rules. Code agents can be understood as the execution core of this Research OS, while humans mainly interact with it through natural language.

This file is an entry point and reference context, not a strict checklist. Start exploring the repository from here, then decide the best implementation based on the actual directory structure, existing documents, current constraints, and your own engineering judgment. You may deviate from the specific suggestions in this file, but explain the reason, impact, and tradeoffs in the final summary.

## Overall Goal

Starting from this file, explore and normalize the Research OS design in this repository so it becomes a simple, efficient, evolvable research template that is easy for code agents to drive.

After normalization, the repository should help agents quickly answer these questions:
...
- `task.md` and `task_en.md` provide direction; they do not replace inspection of the actual repository.
- When `task.md`, `task_en.md`, `INSTRUCTION.md`, `README.md`, `FILETREE.md`, or the actual directory structure conflict, prefer the actual directory structure and `INSTRUCTION.md`, then record the conflict as an item for user decision.
...
- `References/`: Stores papers, web pages, BibTeX, reading notes, and literature indexes.
...
- `Paper_Initial_template/`: Template for starting a new research project or paper draft from a feasible idea. Note that the Git index may show `Paper_initial_template/`; this naming should be unified later.
...
- Pay special attention to inconsistencies or typos such as `Reference` / `References`, `Paper_Initial_template` / `Paper_initial_template`, project template directories, and code documentation filenames.
...
- How a feasible idea should be used to start a new research project or paper draft from `Paper_Initial_template/`.
- How references should enter `References/`, and how to maintain searchable indexes and reading notes.
...
- If changes affect code, configuration, or experiment documentation under `Paper_Initial_template/Code/`, update both the English and Chinese README files there.
...
5. A list of items that still require user decision, such as whether to unify directory casing, whether a CLI is needed, whether to use a more structured index format, and whether HTML should remain a primary delivery format.
